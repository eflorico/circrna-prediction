...
if __name__ == '__main__':
    ...
    train, test = list(kf.split(features))[0]

    rows, row_size = len(train), 5
    num_classes = 2
    ...
    for epoch in range(10):
        for _ in range(100):
            choice = random.sample(train, 10)
            sess.run(model.optimize, {
                data: features[choice], target: labels[choice], dropout: 0.5 })
        error = sess.run(model.error, {
            data: features[test], target: labels[test], dropout: 1})
